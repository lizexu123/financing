...
var pJS = function (tag_id, params) {
    ...
    /* Login&Register.js variables with default values */
    this.pJS = {
        canvas: {
            el: canvas_el,
        ...
        switch (p.shape) {
            ...
            function draw() {
                ...
                if (pJS.tmp.image_type == 'svg') {
                    var image_obj = p.image.obj;
                } else {
                    var image_obj = pJS.tmp.image_obj;
                }
    ...
    pJS.fn.vendors.createSvgImg = function (p) {
        ...
        /* prepare to create image with colored svg */
        var svg = new Blob([coloredSvgXml], {type: 'image/svg+xml;charset=utf-8'}),
            DOMURL = window.URL || window.webkitURL || window,
            url = DOMURL.createObjectURL(svg);

        /* create particle image obj */
        var image = new Image();
    ...
    pJS.fn.vendors.loadImg = function (type) {
        ...
        if (pJS.particles.shape.image.src != '') {
            ...
            if (type == 'svg') {
                ...
                var image = new Image();
                image.addEventListener('load', function () {
                    pJS.tmp.image_obj = image;
                    pJS.fn.vendors.checkBeforeDraw();
                });
                image.src = pJS.particles.shape.image.src;
...
/* ---------- Login&Register.js functions - start ------------ */
...
function cf() {
    ...
    if (!aa.test(ispassword.value)) {
        layer.msg("确认密码只能是英文或数字,长度6到12位")
    }
    if (ispassword.value != password.value) {
        layer.msg('两次密码必须保持一致');
        return false;
    }
    ...
    if (inputCode.length <= 0) {
        var validate = document.getElementById("input1");
        if (validate.value) {
            document.getElementById("at").innerHTML = "";
            return false;
        } else {
            document.getElementById("at").innerHTML = "不能为空！";
            return false;
        }
    } else if (inputCode != code) {
        document.getElementById("at").innerHTML = "验证码输入错误！";
        createCode();
        return false;
    } else {
        document.getElementById("at").innerHTML = "验证码正确！";
    }
